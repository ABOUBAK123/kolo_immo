...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
    x-data="{
        overall: {{ old('rating_overall', 0) }},
        cleanliness: {{ old('rating_cleanliness', 0) }},
        communication: {{ old('rating_communication', 0) }},
        accuracy: {{ old('rating_accuracy', 0) }},
        value: {{ old('rating_value', 0) }},
        location: {{ old('rating_location', 0) }},
        setRating(category, value) { this[category] = value; },
        starColor(category, starIndex) {
            return starIndex <= this[category] ? 'text-yellow-400' : 'text-gray-200';
        },
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <form action="{{ route('reviews.store', $booking) }}" method="POST">
            ...
            <div class="mb-6 text-center">
                ...
                <p class="text-sm text-gray-500 mt-2" x-text="labels[overall] ?? ''"></p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                @foreach([
                    ['key' => 'cleanliness',   'field' => 'rating_cleanliness',   'label' => 'Propreté',                 'icon' => '✨'],
                    ['key' => 'communication', 'field' => 'rating_communication', 'label' => 'Communication',            'icon' => '💬'],
                    ['key' => 'accuracy',      'field' => 'rating_accuracy',      'label' => 'Conformité à l\'annonce',  'icon' => '📋'],
                    ['key' => 'value',         'field' => 'rating_value',         'label' => 'Rapport qualité/prix',     'icon' => '💰'],
                    ['key' => 'location',      'field' => 'rating_location',      'label' => 'Localisation',             'icon' => '📍'],
                ] as $criterion)
                <div class="p-4 bg-gray-50 rounded-xl">
                    ...
                    <input type="hidden" name="{{ $criterion['field'] }}" :value="{{ $criterion['key'] }}">
                    @error($criterion['field'])<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                @endforeach
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="comment" rows="5"
                    placeholder="Décrivez votre expérience: propreté, accueil, confort, quartier... Minimum 20 caractères."
            ...
            <div class="mt-4 p-3 bg-gray-50 rounded-xl flex items-center gap-2">
                ...
                <p class="text-xs text-gray-500">Les avis sont vérifiés et publiés après modération. Ils ne peuvent pas être modifiés après publication.</p>
            </div>
        </form>
    </div>
</div>
