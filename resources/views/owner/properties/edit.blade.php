            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <select name="country" id="edit-country"
                        onchange="var c=this.value,city=document.getElementById('edit-city');if(!city.value){var cap={'Bénin':'Porto-Novo','Burkina Faso':'Ouagadougou','Cap-Vert':'Praia',"Côte d'Ivoire":'Abidjan','Gambie':'Banjul','Ghana':'Accra','Guinée':'Conakry','Guinée-Bissau':'Bissau','Libéria':'Monrovia','Mali':'Bamako','Mauritanie':'Nouakchott','Niger':'Niamey','Nigeria':'Abuja','Sénégal':'Dakar','Sierra Leone':'Freetown','Togo':'Lomé'};city.placeholder=cap[c]||'';};"
                ...
                <div>
                    ...
                    <input type="text" name="city" id="edit-city" value="{{ old('city', $property->city) }}"
                        placeholder="{{ \App\Helpers\WestAfrica::countries()[\App\Helpers\WestAfrica::resolve(old('country', $property->country))] ?? '' }}"
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="address" id="edit-address" value="{{ old('address', $property->address) }}"
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div class="sm:col-span-2">
                    <div class="flex items-center justify-between mb-1.5">
                        ...
                        <button type="button" id="edit-geocode-btn"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <input type="number" step="any" name="latitude" id="edit-field-lat"
                            value="{{ old('latitude', $property->latitude) }}"
                            placeholder="Latitude ex: 5.3497"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <input type="number" step="any" name="longitude" id="edit-field-lng"
                            value="{{ old('longitude', $property->longitude) }}"
                            placeholder="Longitude ex: -4.0167"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <p id="edit-geocode-status" class="text-xs text-gray-400 mt-1">
                        @if($property->latitude && $property->longitude)
                            ✅ Position enregistrée : {{ $property->latitude }}, {{ $property->longitude }}
                        @else
                            Cliquez sur "Géolocaliser" pour remplir automatiquement depuis l'adresse saisie.
                        @endif
                    </p>
                </div>
            </div>
            ...
            <div class="mb-5">
                ...
                <div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
                    ...
                    <div class="relative group">
                        ...
                        <button type="button"
                            onclick="if(confirm('Supprimer cette photo ?')){var f=document.createElement('form');f.method='POST';f.action='{{ route('owner.properties.photos.delete', [$property, $photo]) }}';f.innerHTML='<input name=_token value={{ csrf_token() }}><input name=_method value=DELETE>';document.body.appendChild(f);f.submit();}"
...
async function geocodeAddressEdit() {
    ...
    try {
        ...
        if (data.length > 0) {
            document.getElementById('edit-field-lat').value = parseFloat(data[0].lat).toFixed(6);
            document.getElementById('edit-field-lng').value = parseFloat(data[0].lon).toFixed(6);
