...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 group">
    ...
    <a href="{{ route('properties.show', $property) }}" class="block relative overflow-hidden">
        ...
        <div class="absolute top-3 right-3 flex items-center gap-2">
            ...
            @auth
            @php $isFaved = $property->isFavedBy(Auth::id()); @endphp
            <form action="{{ route('favorites.toggle', $property) }}" method="POST" class="fav-form">
                @csrf
                <button type="submit"
                    class="w-8 h-8 rounded-full flex items-center justify-center shadow-md transition-all fav-btn
                    {{ $isFaved ? 'bg-red-500 text-white' : 'bg-white text-gray-400 hover:text-red-500' }}"
                    title="{{ $isFaved ? 'Retirer des favoris' : 'Ajouter aux favoris' }}">
                    <svg class="w-4 h-4" fill="{{ $isFaved ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-4">
        ...
        <div class="flex items-center justify-between pt-3 border-t border-gray-100">
            <div class="flex items-center gap-1 text-gray-400 text-xs">
                @if($property->owner && $property->owner->isKycVerified())
