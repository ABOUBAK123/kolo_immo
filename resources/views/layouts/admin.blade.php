    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 flex flex-col"
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('admin.payouts.index') }}"
                ...
                Distribution fonds
                @php $pendingPayouts = \App\Models\Booking::where('status','completed')->where('payment_status','escrowed')->count(); @endphp
                @if($pendingPayouts > 0)
                <span class="ml-auto bg-green-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $pendingPayouts }}</span>
                @endif
            </a>
